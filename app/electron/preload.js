...
contextBridge.exposeInMainWorld('noteAPI', {
  ...
  saveImage: (dataUri) => ipcRenderer.invoke('images:save', plain(dataUri)),
  // 实时屏幕截图：列出屏幕/窗口 + 截取画面
  ...
  // 本地音频转写 (Whisper)
  transcribeAudio: (audioData) => ipcRenderer.invoke('audio:transcribe', plain(audioData)),

  // AI 接口（支持流式）
  generateNote: (rawText) => ipcRenderer.invoke('ai:generateNote', plain(rawText)),
  chatWithAI: (question, noteContext, history) => ipcRenderer.invoke('ai:chat', plain(question), plain(noteContext), plain(history)),
